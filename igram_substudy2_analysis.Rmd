...
s_toTest <- s %>%
  ...
  mutate(study_day_consec = study_day) %>%
  
  mutate(had_amox = grepl("Amoxicillin|Amoxicillin-Clavulanate", current_abx)) %>%
  mutate(had_other = grepl(s_abx_summary_SS2 %>% filter(!grepl("Amoxicillin|Amoxicillin-Clavulanate", abx_name)) %>% pull(abx_name) %>% unique() %>% paste(collapse="|"), current_abx)) %>%
  
  mutate(on_abx = ifelse(on_abx, "On abx", "Off abx")) %>%
  mutate(had_other = ifelse(had_other, "Other abx", "No other abx")) %>%
  mutate(had_amox = ifelse(had_amox, "Amox", "No amox")) %>%
  mutate(had_amox = fct_rev(factor(had_amox))) %>%
...
ggsave("igram_abx_SuppFig2_SS2_diffAb_all.pdf", height=6, width=9, useDingbats=F)
```
...
```{r}
summaries_df <- rbind(
  adonisplus(s_toTest, distmat = bc, formula = distmat ~ is_breastfed, 
             sample_id_var = SampleID, rep_meas_var = SubjectID, 
             shuffle = c(is_breastfed = "between"), permutations=999),
  adonisplus(s_toTest, distmat = bc, formula = distmat ~ baseline_age, 
             sample_id_var = SampleID, rep_meas_var = SubjectID, 
             shuffle = c(baseline_age = "between"), permutations=999),
  adonisplus(s_toTest, distmat = bc, formula = distmat ~ study_day, 
             sample_id_var = SampleID, rep_meas_var = SubjectID, 
             shuffle = c(study_day = "within"), permutations=999),
  adonisplus(s_toTest, distmat = bc, formula = distmat ~ SubjectID, 
...
```{r}
adonisplus(s_toTest, distmat = bc, formula = distmat ~ SubjectID + study_day, 
             sample_id_var = SampleID, rep_meas_var = SubjectID, 
             shuffle = c(study_day = "within"), permutations=999) %>%
  pander(split.table=Inf, digits=2)
...
ggsave("igram_abx_SuppFig5_card_penam_SS2.pdf", height=20, width=20, useDingbats=F)
```
